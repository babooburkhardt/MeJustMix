...
object KubelkaMunkColorMixing {
     ...
     * Requires a white reference for calibration.
     */
    fun calculateKSFromSpectral(sample: List<Float>, whiteRef: List<Float>): KSColor? {
        ...
        val ks = reflectance.map { r -> ((1 - r) * (1 - r)) / (2 * r) }
    ...
    /**
     * Converts raw 18-channel spectral data directly to an RGB Int for display.
     */
